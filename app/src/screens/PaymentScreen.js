...
import { View, Text, StyleSheet, TextInput , Button, TouchableWithoutFeedback, ActivityIndicator, Keyboard} from 'react-native'
...
export default function PaymentScreen(props) {
    ...
    var { route: {params}, navigation } = props;
    const { auth } = useAuth()
   ...
   useEffect(() => {   
    ...
    const retriveCustomer = async (key) =>{
        ...
        try {
            ...
            const response = await getClientByloan({searchKey: key, employeeId: auth.employee_id})
            ...
            for(var l of response?.loans){
                ...
                setIsCustomer(false)
                setLoan(key)
                setError(`El cliente con No. préstamo/cédula ${loan} no existe, o no se encuentra entre tus zonas asignadas`)
            }
            ...
            <View>

                {
                   ...
                   <View style={{alignItems: 'center', paddingTop: 40}}>
                       <Text style={{fontSize: 19, fontWeight: 'bold'}}>OH oh... No te ecuentras conectado...</Text>
                       <Text style={{fontSize: 16, color: 'blue', marginTop: 15}} onPress={() => navigation.navigate('Settings')}>Conectarse</Text>
                   </View>  
...
function PaymentCustomerCard(props){
    ...
    const { customer, loans, isCustomer, setIsCustomer, navigation, loan, quotas, register, setIsOpenedComment, isOpenedComment} = props
            ...
            <Card>
                <View style={styles.infoContent}>
                    <View style={styles.row, styles.icon}>
                        <Text style={styles.iconText}>{extractIconText(formatFullName(customer?.first_name, customer))}</Text>
                    </View>
                    <View style={styles.customerInfoContent}>  
                        <Text style={styles.customerInfoName}>{formatFullName(customer?.first_name, customer)}</Text>
                    </View>
                    ...
                    <Menu>
                        <MenuTrigger>
                            <Icon 
                             name='ellipsis-v'
                             style={{top: 0, fontSize: 18, width: 33, paddingHorizontal: 13, paddingVertical: 6, borderRadius: 50}}
                            />
                        </MenuTrigger>
                        <MenuOptions customStyles={{optionText: {fontSize: 15}}} optionsContainerStyle={{marginLeft: 6}}>
                            <MenuOption style={styles.menuOption} onSelect={() => goToPage('PaymentsForm', navigation, {...customer, loans, loan, quotas, register})} text='Cobrar'/>
                            <MenuOption style={styles.menuOption} text='Ver Recibo' onSelect={() => navigation.navigate('Receipt', {customer, loans: loans, loan})} />
                            <MenuOption 
